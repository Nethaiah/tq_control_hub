...
| Receipt/invoice OCR | Treat OCR as an AI Insights workflow: uploaded receipt/invoice photo or PDF is parsed into a draft transaction using LlamaParse, then reviewed before ledger commit. | LlamaParse is purpose-built for extracting structured text/tables from PDFs and scanned financial documents, so it keeps OCR separate from categorization while preserving the PRD rule that AI proposes and humans commit. | Adds another provider and upload-processing step; extracted fields still need confidence scoring and human review because OCR can misread amounts, dates, vendors, or currencies. |
...
| AI with judgement | Achieved for six features | OpenRouter supports CSV categorization, natural-language filter translation, monthly briefing generation, 12-month forecasting, and receipt field extraction fallback with confidence, fallbacks, and no direct ledger mutation. Receipt/invoice OCR sits in AI Insights as a draft-transaction workflow: LlamaParse extracts document text from uploaded photos/PDFs using the Agentic tier with camera-photo correction, then the same review/confirm boundary applies before anything touches the ledger. | AI suggestion action buttons (Confirm/Apply, Dismiss) are hidden when `reviewState` is `applied` or `dismissed`; OCR adds provider cost/latency and must not bypass human confirmation. |
...
| MVP notes | Contextual only | MVP notes appear inside specific feature cards (categories, settings) rather than a global dashboard banner. The OCR card now has an upload-to-draft flow and still labels the no-silent-commit boundary. | Keeps the dashboard clean while surfacing gaps where they matter. |
